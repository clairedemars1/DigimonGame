...
FrameFactory::~FrameFactory() {
  ...
  std::map<std::string, std::vector<TextureWrapper*> >::iterator
...
TextureWrapper* FrameFactory::getFrame(const std::string& name) {
    std::map<std::string, TextureWrapper*>::const_iterator it = frames.find(name);
  if ( it == frames.end() ) {
    SDL_Texture * const texture =
      IOmod::getInstance().readTexture( gdata.getXmlStr(name+"/file"));
    textures[name] = texture;
    TextureWrapper * const frame =new TextureWrapper(texture);
    frames[name] = frame;
    return frame;
  }
  else {
...
std::vector<TextureWrapper*> FrameFactory::getFrames(const std::string& name) {
  // First search map to see if we've already made it:
  std::map<std::string, std::vector<TextureWrapper*> >::const_iterator
  ...
  for (unsigned i = 0; i < numberOfFrames; ++i) {
    ...
    frames.push_back( new TextureWrapper(tex) );
    SDL_FreeSurface(sub);
  }
