...
void IOmod::setFontSize(int newSize){
	TTF_CloseFont(font);
	font = TTF_OpenFont(Gamedata::getInstance().getXmlStr("font/file").c_str(), newSize);
	if (font == NULL) {
		throw std::string("error: font size could not be changed, font not found.");
	}
}

void IOmod::reSetFontSize(){
	TTF_CloseFont(font);
	font = TTF_OpenFont(Gamedata::getInstance().getXmlStr("font/file").c_str(), Gamedata::getInstance().getXmlInt("font/size"));
	if (font == NULL) {
		throw std::string("error: font size could not be changed, font not found.");
	}
}
